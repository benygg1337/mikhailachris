...
for (let index = 0; index < popups.length; index++) {
  ...
  // Проверка активности попапа и его закрытие
  function checkAndClosePopup() {
    const popup = document.querySelector('.popup_thanks._active');
    const popup2 = document.querySelector('.popup_in9-form._active');
    if (popup) {
  ...
  // Слушатель клика по всему окну
  window.addEventListener('click', function () {
    checkAndClosePopup();

  });

  // Слушатель тапа на мобильных устройствах
  window.addEventListener('touchstart', function () {
    checkAndClosePopup();
  });

  // // Таймер для перезагрузки страницы, если в течение 3 секунд не было кликов или тапов
  // setTimeout(() => {
  //   checkAndClosePopup();
  // }, 0);
...
function showPopup(attendanceStatus) {
  ...
  const inputno = document.querySelector('.in9-form-input-no');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function formSend(e) {
          ...
          fetch(formAction, {
            ...
            .then((response) => {
              if (response.result === 'success') {
                ...
                document.querySelector('.popup_in9-form').classList.toggle('_active');
      ...
      }, 2000); // 3000 миллисекунд = 3 секунды, можно изменить по необходимости
              } else {
    ...
    async function initializeRecaptcha() {
      return new Promise((resolve, reject) => {
        grecaptcha.enterprise.ready(() => {
          grecaptcha.enterprise.execute('6LcEYu0pAAAAAID87Wn-jeSSxObP1xRL70d2-W_R', { action: 'submit' }).then((token) => {
